...
- **Tests:** 82 tests, all green (2026-08-27), TWO assemblies: `STS.Core.Tests` 69 (engine+CardTextFormatter; `Assets/Scripts/Tests/EditMode/`, shared fixtures in `TestContent.cs`) + `STS.Content.Tests` 13 (content gate; `Assets/Scripts/Tests/Content/`: JSON schema/reference validation, bad-data rejection, every-encounter and every-card simulated battles on real data). **Run BOTH: TestRunner filter assemblyNames = ["STS.Core.Tests", "STS.Content.Tests"].**
